...
import { Mail, Lock, User as UserIcon, Chrome, Apple, Check, Eye, EyeOff } from "lucide-react-native";
...
export default function WelcomeScreen() {
  ...
  const [, response, promptAsync] = Google.useAuthRequest({
    clientId: "555324826593-2skrpffhfgo7k1h4hubp4v26m66312oc.apps.googleusercontent.com",
  ...
  useEffect(() => {
    if (response?.type === "success") {
      ...
      completeWelcome()
        .then(() => router.replace("/onboarding" as any))
        .catch((e) => console.error("Post-auth error", e));
    } else if (response?.type === "error") {
      console.error("Google auth error", response.error);
    }
  }, [response, completeWelcome, router]);
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (res.type === "success") {
        return; // handled by response effect
      }
      if (res.type === "error") {
        console.error("Google prompt error", res.error);
      }
    } catch (error) {
